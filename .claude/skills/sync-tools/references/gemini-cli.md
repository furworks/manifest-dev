...
9. **Model tier routing is Claude Code-only** — `execution-modes/efficient.md` references Claude model names (haiku, sonnet, opus). Replace all with `inherit` during sync. Gemini supports `model: inherit` natively. Execution mode parallelism, loop limits, and gate-skipping still apply.
