...
**Efficient mode**: When a criterion fails twice at inherit, auto-escalate that criterion's verifier to inherit (session model). Track total escalations — after 3 in a single /do run, suggest to the user: "Efficient mode is escalating frequently. Consider switching to balanced." This prevents runaway costs from repeated escalation.
