...
Session data is stored in `~/.local/share/opencode/opencode.db` (SQLite). Access via the SDK client API. Find recent sessions containing `/define` activity. If `$ARGUMENTS` specifies a session count, use that; otherwise use enough recent sessions for meaningful pattern signal.
